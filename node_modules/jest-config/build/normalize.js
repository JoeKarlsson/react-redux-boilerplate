...
function _load_crypto() {return _crypto = _interopRequireDefault(require('crypto'));}var _glob;
function _load_glob() {return _glob = _interopRequireDefault(require('glob'));}var _path;
function _load_path() {return _path = _interopRequireDefault(require('path'));}var _jestValidate;
function _load_jestValidate() {return _jestValidate = require('jest-validate');}var _validate_pattern;
function _load_validate_pattern() {return _validate_pattern = _interopRequireDefault(require('./validate_pattern'));}var _jestUtil;
function _load_jestUtil() {return _jestUtil = require('jest-util');}var _chalk;
function _load_chalk() {return _chalk = _interopRequireDefault(require('chalk'));}var _get_max_workers;
function _load_get_max_workers() {return _get_max_workers = _interopRequireDefault(require('./get_max_workers'));}var _jestResolve;
function _load_jestResolve() {return _jestResolve = _interopRequireDefault(require('jest-resolve'));}var _jestRegexUtil;
function _load_jestRegexUtil() {return _jestRegexUtil = _interopRequireDefault(require('jest-regex-util'));}var _utils;
function _load_utils() {return _utils = require('./utils');}var _constants;
...
function _load_constants() {return _constants = require('./constants');}var _reporter_validation_errors;




function _load_reporter_validation_errors() {return _reporter_validation_errors = require('./reporter_validation_errors');}var _defaults;
function _load_defaults() {return _defaults = _interopRequireDefault(require('./defaults'));}var _deprecated;
function _load_deprecated() {return _deprecated = _interopRequireDefault(require('./deprecated'));}var _set_from_argv;
function _load_set_from_argv() {return _set_from_argv = _interopRequireDefault(require('./set_from_argv'));}var _valid_config;
function _load_valid_config() {return _valid_config = _interopRequireDefault(require('./valid_config'));}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
const ERROR = `${(_utils || _load_utils()).BULLET}Validation Error`; /**
                                                                      ...
                                                                      *
                                                                      * 
                                                                      */const JSON_EXTENSION = '.json';const PRESET_NAME = 'jest-preset' + JSON_EXTENSION;const createConfigError = message => new (_jestValidate || _load_jestValidate()).ValidationError(ERROR, message, (_utils || _load_utils()).DOCUMENTATION_NOTE);const setupPreset = (options,
optionsPreset) =>
{
  let preset;
  const presetPath = (0, (_utils || _load_utils())._replaceRootDirInPath)(options.rootDir, optionsPreset);
  const presetModule = (_jestResolve || _load_jestResolve()).default.findNodeModule(
...
const setupBabelJest = options => {
  ...
  if (transform) {
    ...
    if (customJSPattern) {
      const jsTransformer = (_jestResolve || _load_jestResolve()).default.findNodeModule(
      transform[customJSPattern],
      { basedir });

      if (
      jsTransformer &&
      jsTransformer.includes((_constants || _load_constants()).NODE_MODULES + 'babel-jest'))
      {
...
key) =>
{
  ...
  (_jestRegexUtil || _load_jestRegexUtil()).default.replacePathSepForRegex(
...
const normalizePreprocessor = options => {
  if (options.scriptPreprocessor && options.transform) {
    ...
    `  Options: ${(_chalk || _load_chalk()).default.bold('scriptPreprocessor')} and ${(_chalk || _load_chalk()).default.bold(
    'transform')
  ...
  if (options.preprocessorIgnorePatterns && options.transformIgnorePatterns) {
    throw createConfigError(
    `  Options ${(_chalk || _load_chalk()).default.bold('preprocessorIgnorePatterns')} and ${(_chalk || _load_chalk()).default.bold(
    'transformIgnorePatterns')
    } cannot be used together.
  Please change your configuration to only use ${(_chalk || _load_chalk()).default.bold(
    'transformIgnorePatterns')
    }.`);
...
const normalizeRootDir = options => {
  ...
  if (!options.hasOwnProperty('rootDir')) {
    throw createConfigError(
    `  Configuration option ${(_chalk || _load_chalk()).default.bold('rootDir')} must be specified.`);
...
const normalizeReporters = (options, basedir) => {
  ...
  options.reporters = reporters.map(reporterConfig => {
    ...
    const reporterPath = (0, (_utils || _load_utils())._replaceRootDirInPath)(
    options.rootDir,
...
const buildTestPathPattern = argv => {
  if (argv.testPathPattern) {
    if ((0, (_validate_pattern || _load_validate_pattern()).default)(argv.testPathPattern)) {
...
function normalize(options, argv) {var _validate =
  (0, (_jestValidate || _load_jestValidate()).validate)(options, {
    comment: (_utils || _load_utils()).DOCUMENTATION_NOTE,
    deprecatedConfig: (_deprecated || _load_deprecated()).default,
    exampleConfig: (_valid_config || _load_valid_config()).default });const hasDeprecationWarnings = _validate.hasDeprecationWarnings;


  options = normalizePreprocessor(
  normalizeReporters(
  normalizeMissingOptions(normalizeRootDir((0, (_set_from_argv || _load_set_from_argv()).default)(options, argv)))));
    ...
    switch (key) {
      ...
      case 'projects':
        ...
        reduce((projects, project) => {
          ...
          const globMatches = (_glob || _load_glob()).default.sync(project);
          return projects.concat(globMatches.length ? globMatches : project);
        }, []);
        break;
      case 'moduleDirectories':
      case 'testMatch':
        value = (0, (_utils || _load_utils())._replaceRootDirTags)(options.rootDir, options[key]);
        break;
      case 'automock':
      case 'bail':
      case 'browser':
